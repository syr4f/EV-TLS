...
#include "evuitm/TraciTrafficLightLogic.h"

using namespace veins;
using namespace evuitm;

Define_Module(evuitm::TraciTrafficLightLogic);

TraciTrafficLightLogic::TraciTrafficLightLogic()
...
void TraciTrafficLightLogic::initialize() {
...
void TraciTrafficLightLogic::handleApplMsg(cMessage* msg) {
    auto* applMsg = check_and_cast<TrafficLightBeacon*>(msg);
    ...
    switch (msg_type) {
        ...
        default: {
            EV_WARN << "TrafficLightLogic: LOGIC: Error, got message from APPL of unknown type";
        }
    }
    delete msg;
}

void TraciTrafficLightLogic::handleTlIfMsg(TraCITrafficLightMessage* tlMsg) {
    delete tlMsg;
}

void TraciTrafficLightLogic::handlePossibleSwitch() {
}

void TraciTrafficLightLogic::handleSelfMsg(cMessage* msg) {
    if (timerManager.handleMessage(msg))
        return;
    TraCITrafficLightAbstractLogic::handleSelfMsg(msg);
}

void TraciTrafficLightLogic::startPreemption(const std::vector<int>& ev_indices) {
...
void TraciTrafficLightLogic::switchProgramBack(){
    if (!tl_switched_back){
...
void TraciTrafficLightLogic::scheduleStartPreemptionMsg(PreemptionRequestSource kind, simtime_t TP, const std::vector<int>& indices){
...
bool TraciTrafficLightLogic::isPreemptionAllowed(std::string program_id, int phase_id){
...
simtime_t TraciTrafficLightLogic::getActualPhaseTime() {
...
std::vector<int> TraciTrafficLightLogic::getIndicesOfGreensFromPhase(std::string phase){
    std::vector<int> ret;
    int i = 0;
    for(auto& s : phase){
        if (s=='g' || s=='G')
...
TraCITrafficLightProgram::Logic TraciTrafficLightLogic::createPreemptionProgram(
...
std::string TraciTrafficLightLogic::getDesiredPhase(const std::vector<int>& indices, int len){
