...
#include "evuitm/TraciTrafficLightApp.h"
#include <algorithm>

using namespace veins;
using namespace evuitm;

Define_Module(evuitm::TraciTrafficLightApp);

void TraciTrafficLightApp::initialize(int stage)
{
...
std::vector<int> TraciTrafficLightApp::initIndicesEvWay(){
...
void TraciTrafficLightApp::finish()
{
...
void TraciTrafficLightApp::handleSelfMsg(cMessage* msg) {
    if(timerManager.handleMessage(msg))
        return;
    DemoBaseApplLayer::handleSelfMsg(msg);
}

void TraciTrafficLightApp::instructLogicToStartPreemptionIn(double timeOffset) {
    TrafficLightBeacon* msg = generateApplToLogicMsg(timeOffset, START_PREEMPTION);
    send(msg, "logic$o");
}

void TraciTrafficLightApp::instructLogicToStopPreemption() {
    TrafficLightBeacon* msg = generateApplToLogicMsg(0.0, STOP_PREEMPTION);
    send(msg, "logic$o");
}

TrafficLightBeacon* TraciTrafficLightApp::generateApplToLogicMsg(simtime_t content, int type) {
    std::string msgName = tl_id + "_MsgToLogic";
    TrafficLightBeacon* ret = new TrafficLightBeacon(msgName.c_str());
...
bool TraciTrafficLightApp::isEvWayCurrentlyGreen() {
...
double TraciTrafficLightApp::getNewTG(){
    bool is_green = isEvWayCurrentlyGreen();
    return is_green ? getActualPhaseTime() : 0.0;
}

double TraciTrafficLightApp::getActualPhaseTime(){
...
int TraciTrafficLightApp::getNumOfVehiclesFromDetectors(){
...
double TraciTrafficLightApp::computeDistanceToTraffiLightDownstream(veins::Coord emveh_pos){
